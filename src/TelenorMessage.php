...
class TelenorMessage
{
    ...
    public function __construct($content = ''){
        ...
        $this->payload['sender'] = [
            '@type' => '5',
            'name' => config('telenorsms.sms.sender')
        ];
        $this->payload['sendTime'] = date('Y-m-d\TH:i:sP');
        $this->payload['characteristic'][] = ['name' => 'UserName', 'value' => config('telenorsms.sms.username')];
        $this->payload['characteristic'][] = ['name' => 'Password', 'value' => config('telenorsms.sms.password')];
    }
